...
public class Teclado {
	...
	/**
	 * Muestra por pantalla un menú y lee por teclado. Controla que el valor
	 * devuelto este entre el rango del menú
	 * 
	 * @param args
	 *            Vector de cadena con las distintas posiblidades del menu
	 * @param min
	 *            Mínimo valor del menú
	 * @param max
	 *            Máximo valor del menú
	...
	public int Menu(String[] args, int min, int max) throws IOException {
		int opcion = 0;
		do {
			try { // Try de la excepción por si se introduce una letra en vez de
					// un número en el menú o no se introduce nada
				...
				// opcion = Integer.parseInt(Input.readLine()); //Convertimos lo
				// que leemos por teclado y lo convertimos a entero
				opcion = leerEntero();
				if (!CV)
					System.out.print("\n");

			} catch (NumberFormatException e) { // Excepción por si se introduce
												// una letra en vez de un número
												// en el menú o no se introduce
												// nada
				if (!CV)
